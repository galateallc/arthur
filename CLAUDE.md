...
- **Web UI**: mobile-responsive, installable PWA (manifest + icon in `web_assets/`); `Dockerfile` + `KINDALIVE_HOST`/`$PORT` for always-on hosting. Header Reset rebuilds the robot with a slightly-randomized chemical state (±`RESET_JITTER`=0.22 per chemical, also applied at server start) and a fresh conversation. Companion API: `POST /api/say` (`{"text": "..."}`) feeds external conversations through the same pipeline as the textarea, so an outside chat (hook, browser extension, custom GPT action) can drive the face; the robot reacts with its own feelings — it never mirrors the speaker. The open page surfaces API-injected exchanges on its next tick (status line, reply bubble, TTS). `GET /face.png` (dependency-free renderer in `expression/face_image.py`, same dot geometry as `face3d.js`) serves the current expression as an image; `/api/say` returns a cache-busted absolute `face_url` so image-only integrations can embed the face each turn.
